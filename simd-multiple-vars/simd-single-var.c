...
int main(int argc, char* argv[])
{
  ...
  // Computation (laplacian-like) with a single iteration var and shifted indices
  printf("Now computing %d steps with iter var z\n", nsteps);
  ...
  for (int t = 0, t0 = t%2, t1 = (t + 1)%2; t < nsteps; t++, t0 = t%2, t1 = (t + 1)%2)
  {
    for (int x = 0; x < size; x++)
    {
      for (int y = 0; y < size; y++)
      {
        ...
        {
          r[x + 2][y + 2][z + 1] = u[t0][x][y][z + 1] - u[t0][x][y][z + 2] + u[t0][x][y][z + 3];
        }
      }
    }
    for (int x = 0; x < size; x++)
      ...
      for (int y = 0; y < size; y++)
      {
        ...
        for (int z = 0; z < size; z++)
        {
          u[t1][x + 2][y + 2][z + 2] = 
              ...
              (r[x + 2][y + 2][z] + r[x + 2][y + 2][z + 1] + r[x + 2][y + 2][z + 2] + r[x + 3][y + 2][z] + r[x + 3][y + 2][z + 1])*0.001 + 0.00000001;
       }
      }
    }
  }
